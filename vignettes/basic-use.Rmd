...
- `installTo` is the directory your app will be built into (where you will find the `exe`, etc.)
...
In the directory your specified in `installTo`, you will find a new folder named what you chose for `appName`. Directly inside you will find the raw electron build. Your new electron `.exe` can be found in `installTo/appName/dist`, where `appName` is what you assigned `buildElectricApp(appName = )`. 


Advanced: If you want to test the electron app without installing. You can open cmd.exe and `cd` to `installTo/appName` and if you have nodejs installed and in your `PATH`, use the command `npm start` to initiate the app. 

Advanced: If you want to make changes before creating the exe- run the `electricShine::buildElectricApp()` with `build = FALSE` and make your changes within `installTo/appName`. When you're ready to build the installer run the following, using the same `installTo` and `appName` as in `electricShine::buildElectricApp()`.

```{r}
appPath <- file.path(installTo, appName)
